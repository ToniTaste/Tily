    ...
    class Board {
        ...
        fillCell(r, c, color) {
            if (r >= 0 && c >= 0 && r < this.rows && c < this.cols) {
                this.fillColors[`${r},${c}`] = color;
                this.draw();
            }
        }
        getCellRect(r, c) {
            return {
                x: this.margin + c * this.cellW,
                y: this.margin + r * this.cellH,
                w: this.cellW,
                h: this.cellH
            };
        }
        ...
        draw() {
            ...
            for (let r = 0; r < this.rows; r++) {
                for (let c = 0; c < this.cols; c++) {
                    ...
                    const key = `${r},${c}`;
                    if (this.fillColors[key]) {
                        ctx.fillStyle = this.fillColors[key];
                        ctx.fillRect(x, y, w, h);
                    }
                    ctx.strokeStyle = "#999";
        ...
        exportPortableBitmap() {
            ...
            const used = new Set(
                    Object.values(this.fillColors).map(c => normalize(c)));

            used.add("#ffffff");

            // 1) PBM?
            // Nur wenn genau zwei Farben vorkommen und eine davon Weiß ist.
            // Weiß wird als 0 gespeichert, die andere Farbe als 1.
            if (used.size === 2 && used.has("#ffffff")) {
                const black = Array.from(used).find(c => c !== "#ffffff");
